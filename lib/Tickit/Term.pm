...
use Term::Size;
use Term::TermKey 0.11 qw( FORMAT_ALTISMETA FLAG_UTF8 FLAG_RAW FLAG_EINTR RES_KEY RES_AGAIN CANON_DELBS );
use Term::Terminfo;
...
sub new
{
   ...
   $termkey->set_canonflags( $termkey->get_canonflags | CANON_DELBS );
...
sub _got_key
{
   ...
   if( $key->type_is_unicode and !$key->modifiers ) {
      ...
      if( $button > 3 ) {
         $evname = "wheel";
         $button = (qw( up down ))[$button - 4] or return;
      }
